...
export default function HomePage() {
      ...
      <div className="auth-screen">
        <div className="auth-card">
          ...
          <div className="auth-form-stack">
            ...
            {authMode === 'register' && (
              <div className="form-group">
                <label className="form-label">Display name <span style={{ color: 'var(--text-3)' }}>(optional)</span></label>
                <input className="form-input" placeholder="Your name" value={name} onChange={e => setName(e.target.value)} />
              </div>
            )}
          </div>
          {authMsg && (
            <div className={`alert ${authMsg.includes('failed') || authMsg.includes('required') ? 'alert-error' : 'alert-success'}`}>
              {authMsg}
            </div>
          )}
          <button className="btn btn-primary w-full" style={{ justifyContent: 'center', padding: '10px', fontSize: '14px' }} onClick={onAuthSubmit}>
  ...
  /* ─── MAIN LAYOUT ─── */
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="layout futuristic-2026"><div className="cyber-grid" />
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          {navItems.map(item => (
            <button key={item.tab} className={`nav-item${activeTab === item.tab ? ' active' : ''}`} onClick={() => setActiveTab(item.tab)}>
              <Ic n={item.icon} s={14} />
        ...
        <div className="sidebar-bottom">
          ...
          <button className="nav-item" style={{ color: 'var(--error)', opacity: 0.85 }} onClick={onLogout}>
            <Ic n="logout" s={14} />
            Sign out
          </button>
        </div>
      ...
      <div className="main-area">
        ...
        <header className="topbar">
          ...
          <div className="tb-right">
            {error && <span style={{ fontSize: 12, color: 'var(--error)', maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{error}</span>}
        ...
        <div className="content">
            ...
            <div>
              <div className="page-hd">
                ...
                <div className="page-actions">
                  ...
                  <button className="btn btn-primary" onClick={() => { setEditingFlowId(''); setFlowName(''); setActiveTab('builder'); }}>
              ...
              <div className="stat-grid">
                <div className="stat-card">
                  <div className="stat-icon" style={{ background: 'rgba(79,126,255,0.12)', color: 'var(--accent)' }}><Ic n="zap" s={16} /></div>
                  <div className="stat-label">Total Flows</div>
                  <div className="stat-val" style={{ color: 'var(--accent)' }}>{flows.length}</div>
                  <div className="stat-hint">{flows.filter(f => f.enabled).length} enabled</div>
                </div>
                <div className="stat-card">
                  <div className="stat-icon" style={{ background: 'rgba(245,158,11,0.1)', color: 'var(--warning)' }}><Ic n="activity" s={16} /></div>
                  <div className="stat-label">Total Runs</div>
                  <div className="stat-val">{runs.length}</div>
                  <div className="stat-hint">all time</div>
                </div>
                <div className="stat-card">
                  <div className="stat-icon" style={{ background: 'var(--success-subtle)', color: 'var(--success)' }}><Ic n="check" s={16} /></div>
                  <div className="stat-label">Success Rate</div>
                  <div className="stat-val" style={{ color: runs.length === 0 ? 'var(--text-2)' : successRate >= 80 ? 'var(--success)' : 'var(--warning)' }}>
                    {runs.length > 0 ? `${successRate}%` : '—'}
                  </div>
                  <div className="stat-hint">{successRuns} ok · {failedRuns} failed</div>
                </div>
                <div className="stat-card">
                  <div className="stat-icon" style={{ background: 'var(--success-subtle)', color: 'var(--success)' }}><Ic n="activity" s={16} /></div>
                  <div className="stat-label">Live Events</div>
                  <div className="stat-val" style={{ color: 'var(--success)' }}>{events.length}</div>
                  <div className="stat-hint"><span className="live-dot" style={{ display: 'inline-block', marginRight: 4 }} />streaming</div>
                </div>
              </div>
              ...
              <div className="two-col">
                ...
                <div className="card">
                  <div className="card-hd">
                    <div className="card-title"><Ic n="activity" s={14} />Recent Runs</div>
                  </div>
                  {runs.length === 0 ? (
                ...
                <div className="card">
                  <div className="card-hd">
                    <div className="card-title"><span className="live-dot" />Live Events</div>
                    <span className="live-badge"><span className="live-dot" />Live</span>
                  </div>
                  <div style={{ padding: '4px 16px', maxHeight: 300, overflowY: 'auto' }}>
                    {events.length === 0 ? (
                      <div className="empty-state">
                        <div className="empty-icon"><Ic n="activity" s={18} /></div>
          ...
          {activeTab === 'flows' && (
            <div>
              <div className="page-hd">
                ...
                <div className="page-actions">
                  <button className="btn btn-secondary" onClick={loadFlows}><Ic n="refresh" s={13} />Refresh</button>
                  <button className="btn btn-primary" onClick={() => { setEditingFlowId(''); setFlowName(''); setFlowDescription(''); setFlowEnabled(true); setDefinitionJson(JSON.stringify(buildFlowDefinition(), null, 2)); setActiveTab('builder'); }}>
                    <Ic n="plus" s={13} />New Flow
                  </button>
                </div>
              </div>
                    ...
                    <tbody>
                      {flows.map(flow => (
                        <tr key={flow.id}>
                          ...
                          <td><span className="text-muted">{flow.run_count}</span></td>
            ...
            <div>
              <div className="page-hd">
                ...
                <div className="page-actions">
                  ...
                  <button className="btn btn-primary" onClick={saveFlow}><Ic n="save" s={13} />{editingFlowId ? 'Update Flow' : 'Create Flow'}</button>
                </div>
              </div>
              ...
              <div className="builder-grid">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div className="card">
                    <div className="card-hd"><div className="card-title">Flow Details</div></div>
                    <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <label className="form-check-row">
                        ...
                        <span style={{ fontSize: 13.5, fontWeight: 500 }}>Enabled</span>
                      </label>
                    </div>
                  </div>

                  <div className="card">
                    <div className="card-hd"><div className="card-title">Trigger</div></div>
                  ...
                  <div className="card">
                    <div className="card-hd"><div className="card-title">Action Step</div></div>
                    <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      <div className="form-group">
                        <label className="form-label">Connector</label>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div className="card">
                    <div className="card-hd"><div className="card-title"><Ic n="grid" s={14} />Visual Canvas</div></div>
                    <div style={{ padding: 14 }}>
                      <FlowCanvas definitionJson={definitionJson} onDefinitionJsonChange={setDefinitionJson} connectors={connectors} />
                    </div>
                  </div>
                  <div className="card">
                    <div className="card-hd"><div className="card-title">Flow Definition JSON</div></div>
                    <div style={{ padding: 14 }}>
                      <textarea
          ...
          {activeTab === 'credentials' && (
            <div>
              ...
              <div className="two-col" style={{ alignItems: 'start' }}>
                {/* Add credential */}
                <div className="card">
                  <div className="card-hd"><div className="card-title"><Ic n="plus" s={14} />Add Credential</div></div>
                  <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <div className="form-group">
                    ...
                    <button className="btn btn-primary" onClick={saveCredential}><Ic n="save" s={13} />Save Credential</button>
                  </div>
                </div>
                ...
                <div className="card">
                  <div className="card-hd">
                    <div className="card-title"><Ic n="key" s={14} />Stored Credentials</div>
                    <span className="badge b-neutral">{credentials.length}</span>
                  </div>
            ...
            <div>
              ...
              {connectors.length === 0 ? (
                ...
                <div className="connector-grid">
                  {connectors.map(item => (
                    <div key={`${item.connector}:${item.action}`} className="connector-card">
                      ...
                      <div className="connector-meta">
                        ...
                        {item.required_input_fields.length > 0 && (
                          <div style={{ marginTop: 4 }}>
                            Required: <span className="font-mono" style={{ fontSize: 10.5 }}>{item.required_input_fields.join(', ')}</span>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          {activeTab === 'events' && (
            <div>
              ...
              {/* Create Workspace section (moved here for settings-ish stuff) */}
              <div style={{ marginTop: 24 }}>
                <div className="page-hd" style={{ marginBottom: 16 }}>
                  <div>
                    <div className="page-title" style={{ fontSize: 16 }}>Workspaces</div>
                ...
                {showNewWs && (
                  <div className="card mb-16">
                    ...
                    <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
                      ...
                      {workspaceMsg && <div className={`alert ${workspaceMsg.includes('Failed') || workspaceMsg.includes('required') || workspaceMsg.includes('Invalid') ? 'alert-error' : 'alert-success'}`}>{workspaceMsg}</div>}
                      <div className="flex gap-8">
                        <button className="btn btn-primary" onClick={createWorkspace}><Ic n="plus" s={13} />Create Workspace</button>
                          ...
                          <tr key={ws.id}>
                            <td>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <div className="ws-avatar" style={{ width: 22, height: 22 }}>{ws.name.slice(0, 2).toUpperCase()}</div>
