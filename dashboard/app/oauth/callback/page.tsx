  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: 'var(--bg)', color: 'var(--text)', padding: 24 }}>
      <div style={{ width: 'min(640px, 100%)', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18, padding: 28, boxShadow: 'var(--shadow-card)' }}>
        ...
        <button onClick={() => router.replace(`/settings/vault?workspace=${encodeURIComponent(workspaceId)}`)} style={{ padding: '10px 16px', borderRadius: 10, border: '1px solid var(--accent)', background: 'var(--accent)', color: 'white', cursor: 'pointer' }}>
          Return to VaultGuard
        </button>
      </div>
    </main>
  );
